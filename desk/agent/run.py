...
from desk.analyst import _as_lines, _parse_json
...
4. 证据齐了必须 submit_report 结束。不要输出 Markdown。
铁规则：
- 只使用工具返回的数字，禁止编造。
...
def fill_report_with_agent(
    ...
    *,
    ...
    if not session.report:
        session.report = _from_text(loop.content)
    return AgentReport(
        report=_to_sections(session.report),
